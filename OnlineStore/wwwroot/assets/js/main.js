  ...
  $(document).ready(function () {
    ...
    var book = 0;
      $(".seat-free img").on('click', function (e) {
          debugger;
          var choosed = $(".seat-choosed img");
          $.each(choosed, function (i, item) {
              $(item).attr("src", "/assets/images/movie/seat01-free.png");
              $(item).parent().removeClass("seat-choosed");
              $(item).parent().addClass("seat-free");
          })

        $(this).parent().removeClass("seat-free");
        $(this).attr("src", "/assets/images/movie/seat01-booked.png");
        $(this).parent().addClass("seat-choosed");
        $("#selected-seat").html($(this).next().html());
        var moviePrice = Number($("#MoviePrice").val()).toFixed(2);
        $("#movie-price").html(Number($("#MoviePrice").val()).toFixed(2) + ' &euro;');
        var scheduleId = $("#ScheduleId").val();
        var seatId = $(this).next().data("seat-id");
        $.removeCookie('ScheduleId', { path: '/' });
        $.cookie('ScheduleId', scheduleId, { path: '/' });

        $.removeCookie('SeatId', { path: '/' });
        $.cookie('SeatId', seatId, { path: '/' });
    });
    var bookTwo = 1;  
